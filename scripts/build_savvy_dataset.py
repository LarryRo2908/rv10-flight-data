...
from config import (
    DATA_DIR, RAW_CSV_DIR, METADATA_PARQUET, 
    FLIGHT_TELEMETRY_PARQUET, GROUND_TELEMETRY_PARQUET, 
    SESSION_FILE, SAVVY_BASE_URL
)
...
def run_pipeline():
    ...
    try:
        ...
        print("Please run: pip3 install polars pyarrow playwright requests pandas tqdm")
        sys.exit(1)
    ...
    with sync_playwright() as p:
        ...
        if needed_csvs:
            ...
            print("All 76 raw CSV log files are already present locally!")
    ...
    print(f"1. Master Catalog: {METADATA_PARQUET.name} ({METADATA_PARQUET.stat().st_size / 1024:.1f} KB, {len(flights_catalog)} flights)")
